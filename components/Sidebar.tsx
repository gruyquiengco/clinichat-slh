...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, currentView, setView, onLogout, unreadCount }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-viber-purple text-white p-4">
      ...
      <div className="mt-auto pt-4 border-t border-white/10 space-y-4">
        <button onClick={() => setView('profile')} className="w-full flex items-center gap-3 p-2 rounded-2xl hover:bg-white/10 transition-all">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-black shadow-lg ${getRoleStyle(currentUser.role)}`}>
        ...
        <button onClick={onLogout} className="w-full py-2 text-xs font-bold opacity-50 hover:opacity-100 transition-opacity">Sign Out</button>
      </div>
    </div>
  );
};
