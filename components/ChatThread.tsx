...
const ChatThread: React.FC<ChatThreadProps> = ({ 
  ...
  const getPatientInitials = (firstName: string, surname: string) => {
  ...
  const handleLogGeneration = async () => {
    ...
    try {
      // Reassuring delay for heavy processing simulation
      await new Promise(r => setTimeout(r, 800));
      const logContent = await onGenerateSummary(patient, messages);
      ...
      const filename = `CareLog_${patient.surname}_${timestamp}.txt`;
      ...
      setGeneratedLog(logContent);
    } catch (error) {
      console.error("Log export failed:", error);
      alert("Failed to export log. Please try again.");
    } finally {
  ...
  const handleSaveInfo = (e: React.FormEvent) => {
    ...
    const updatedPatient = { 
      ...editFormData, 
      avatarColor: WARD_COLORS[editFormData.ward] || patient.avatarColor 
    };
  ...
  const canEditClinical = isMember && (currentUser.role === UserRole.HCW || currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.SYSCLERK);
    ...
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-viber-dark transition-colors overflow-hidden">
      ...
      <div className="p-3 bg-white dark:bg-viber-dark border-b border-gray-200 dark:border-gray-800 flex items-center justify-between shadow-sm z-20">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div 
            className="w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center font-bold text-lg cursor-pointer transition-transform active:scale-95 shadow-sm"
            style={{ 
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-${isMe ? 'right' : 'left'}-2 duration-200`}>
                ...
                <div className="flex items-end gap-2 max-w-[85%]">
                  {!isMe && (
                    sender?.photo ? (
                      <img src={sender.photo} className="w-6 h-6 rounded-full border border-gray-200 dark:border-gray-700 cursor-pointer" alt="" onClick={() => setSelectedMemberContact(sender || null)} />
                    ) : (
                      <div 
                        className="w-6 h-6 rounded-full flex items-center justify-center text-white font-black text-[7px] cursor-pointer shadow-sm border border-white dark:border-gray-800"
                        style={{ backgroundColor: sender ? (DEPARTMENT_COLORS[sender.department] || '#7360f2') : '#7360f2' }}
                        onClick={() => setSelectedMemberContact(sender || null)}
                      >
      ...
      {showInfo && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/80 z-[100] flex justify-end transition-all animate-in fade-in duration-300">
          <div className="w-full max-sm bg-white dark:bg-gray-900 h-full overflow-y-auto animate-in slide-in-from-right duration-300 shadow-2xl transition-colors pb-20">
            ...
            <div className="p-6 text-center space-y-4">
              ...
              >
                {getPatientInitials(editFormData.firstName, editFormData.surname)}
              </div>
              ...
              {!isEditingInfo ? (
                ...
                <form onSubmit={handleSaveInfo} className="text-left space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5 ml-1">Age</label>
                      <input 
                        type="number"
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5 ml-1">Sex</label>
                      <select 
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5 ml-1">Diagnosis</label>
                    <textarea 
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5 ml-1">Ward</label>
                      <select 
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1.5 ml-1">Room No.</label>
                      <input 
               ...
               <div className="space-y-3">
                 {currentMembers.map(member => (
                    <div key={member.id} className="flex items-center gap-3 p-2 rounded-xl">
                      {member.photo ? (
                        <img src={member.photo} className="w-8 h-8 rounded-full shadow-sm" alt="" />
                      ) : (
                        <div 
                          className="w-8 h-8 rounded-full flex items-center justify-center text-white font-black text-[9px] shadow-sm"
                          style={{ backgroundColor: DEPARTMENT_COLORS[member.department] || '#7360f2' }}
                        >
        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/90 z-[300] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-900 p-8 rounded-[2rem] w-full max-w-xs shadow-2xl text-center">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={handleDischarge} className="w-full py-4 text-sm font-black text-white bg-amber-600 rounded-2xl shadow-xl hover:bg-amber-700 active:scale-95 transition-all">Yes, Discharge Patient</button>
              <button onClick={() => setShowDischargeConfirm(false)} className="w-full py-3 text-sm font-bold text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showLeaveConfirm && (
        <div className="fixed inset-0 bg-black/60 dark:bg-black/90 z-[300] flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-900 p-8 rounded-[2rem] w-full max-w-xs shadow-2xl text-center">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">You will lose access to updates in this clinical thread.</p>
            <div className="flex flex-col gap-3">
              <button onClick={handleLeave} className="w-full py-4 text-sm font-black text-white bg-red-600 rounded-2xl shadow-xl hover:bg-red-700 active:scale-95 transition-all">Yes, Leave Thread</button>
              <button onClick={() => setShowLeaveConfirm(false)} className="w-full py-3 text-sm font-bold text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">Go Back</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[2rem] w-full max-w-xs overflow-hidden shadow-2xl animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            <div className="p-8 text-center border-b dark:border-gray-800">
               {selectedMemberContact.photo ? (
                 ...
                 <div 
                  className="w-24 h-24 rounded-full mx-auto mb-4 border-4 border-white dark:border-gray-800 shadow-md flex items-center justify-center text-white font-black text-2xl"
                  style={{ backgroundColor: DEPARTMENT_COLORS[selectedMemberContact.department] || '#7360f2' }}
                 >
                   {getUserInitials(selectedMemberContact)}
                 </div>
               )}
            ...
            <div className="p-6 grid grid-cols-3 gap-3 border-b dark:border-gray-800">
               <a href={`tel:${selectedMemberContact.phone}`} className="flex flex-col items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
               ...
               <a href={`viber://chat?number=${sanitizePhoneForLink(selectedMemberContact.phone)}`} className="flex flex-col items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
               ...
               <a href={`mailto:${selectedMemberContact.email}`} className="flex flex-col items-center gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl">
            ...
            <div className="p-4"><button onClick={() => setSelectedMemberContact(null)} className="w-full py-3 text-xs font-black text-gray-400 uppercase hover:text-gray-600 transition-colors">Dismiss</button></div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] w-full max-sm overflow-hidden shadow-2xl transition-colors">
            ...
            <div className="max-h-96 overflow-y-auto p-4 space-y-2">
               {users.filter(u => !patient.members.includes(u.id)).map(user => (
                  <button key={user.id} onClick={() => { onAddMember(user.id); setShowAddMember(false); }} className="w-full flex items-center gap-3 p-3 hover:bg-purple-50 dark:hover:bg-purple-900/10 rounded-2xl group">
                    {user.photo ? (
                      <img src={user.photo} className="w-10 h-10 rounded-full" alt="" />
                    ) : (
                      <div 
                        className="w-10 h-10 rounded-full flex items-center justify-center text-white font-black text-[8px] shadow-sm"
                        style={{ backgroundColor: DEPARTMENT_COLORS[user.department] || '#7360f2' }}
                      >
