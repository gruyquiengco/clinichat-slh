...
interface ChatThreadProps {
  ...
  onDischarge: () => void; // Changed to match your App.tsx
            ...
            <div className="space-y-4">
              <div className="flex flex-col items-center pb-6 border-b dark:border-gray-800">
                ...
                {isEditing ? (
                  <div className="space-y-2 w-full">
                    <input className="w-full p-2 text-center border rounded dark:bg-gray-800 dark:text-white" value={editedPatient.surname} onChange={e => setEditedPatient({...editedPatient, surname: e.target.value})} />
                    <input className="w-full p-2 text-center border rounded dark:bg-gray-800 dark:text-white" value={editedPatient.firstName} onChange={e => setEditedPatient({...editedPatient, firstName: e.target.value})} />
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl">
                  ...
                  {isEditing ? <input className="w-full bg-transparent font-bold dark:text-white" type="number" value={editedPatient.age} onChange={e => setEditedPatient({...editedPatient, age: parseInt(e.target.value)})} /> : <p className="text-sm font-bold dark:text-white">{patient.age}</p>}
                </div>
                <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl">
                  <p className="text-[9px] font-black text-gray-400 uppercase">Sex</p>
                  {isEditing ? <input className="w-full bg-transparent font-bold dark:text-white" value={editedPatient.sex} onChange={e => setEditedPatient({...editedPatient, sex: e.target.value as any})} /> : <p className="text-sm font-bold dark:text-white">{patient.sex}</p>}
                </div>
              </div>
              ...
              <div className="bg-orange-50 dark:bg-orange-900/10 p-4 rounded-2xl border border-orange-100">
                ...
                {isEditing ? <textarea className="w-full bg-transparent font-bold dark:text-white" value={editedPatient.diagnosis} onChange={e => setEditedPatient({...editedPatient, diagnosis: e.target.value})} /> : <p className="text-sm font-bold text-orange-900 dark:text-orange-200">{patient.diagnosis}</p>}
              </div>

              <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl">
                <p className="text-[9px] font-black text-gray-400 uppercase mb-1">Ward & Room</p>
                {isEditing ? <input className="w-full bg-transparent font-bold dark:text-white" value={editedPatient.roomNumber} onChange={e => setEditedPatient({...editedPatient, roomNumber: e.target.value})} /> : <p className="text-sm font-bold dark:text-white">{patient.ward} — Room {patient.roomNumber}</p>}
              </div>
            </div>
